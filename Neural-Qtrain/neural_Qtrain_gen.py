...
#    loss = tf.reduce_mean(tf.squared_difference(target_in, q_values))
...
def get_env_action(action):
    ...
    """
#    action = dic_[action]
#    action = (action-(ACTION_SPACE-1)/2)/((ACTION_SPACE-1)/4)
...
#    action = tf.one_hot(action, 2, 1.0, 0.0)
    ...
    action = l
#    action = tf.one_hot(action,action_dim)
...
def do_train_step(replay_buffer, state_in, action_in, target_in,
    ...
    target_batch, state_batch, action_batch,next_state_batch = \
...
#    print(Q_value_batch)
    Q_value_batch_ = q_values.eval(feed_dict={
        state_in: state_batch
    })

    target_batch = []
    max_act4next = np.argmax(Q_value_batch_, axis=1)
#    max_q_values = q_val.max(axis=1)
    for i in range(0, BATCH_SIZE):
        
        sample_is_done = minibatch[i][4]
        if sample_is_done == True:
#            print("d")
            target_batch.append(reward_batch[i])
        else:
            # TO IMPLEMENT: set the target_val to the correct Q value update
#            max_act4next = np.argmax(q_eval[i], axis=1)
#            print("not")
            target_val = reward_batch[i]
            target_val += GAMMA * Q_value_batch[i][max_act4next[i]]
...
#    action_batch = tf.TensorArray(action_batch,[2,BATCH_SIZE],dtype=float32)
    return target_batch, state_batch, action_batch, next_state_batch
